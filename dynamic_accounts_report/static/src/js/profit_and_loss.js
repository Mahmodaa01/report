...
class ProfitAndLoss extends owl.Component {
    ...
    async load_data() {
        ...
        try {
            ...
            console.log("DEBUG: data[1] (filter_data):", data[1]);
            console.log("DEBUG: data[2] (datas):", data[2]);
            if (data && Array.isArray(data)) {
                self.state.data = data[0] || null;
                self.state.datas = data[2] || [];
                self.state.filter_data = data[1] || null;
                self.state.title = action_title;

                // Set initial_render to false after the first successful data load
                ...
                self.state.datas = [];
                self.state.filter_data = null;
                self.state.title = action_title;
            }
        }
        catch (el) {
            console.error("Error loading data:", el);
            self.state.data = null;
            self.state.datas = [];
            self.state.filter_data = null;
            self.state.title = action_title;
